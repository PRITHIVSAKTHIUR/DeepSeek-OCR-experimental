...
model_name = "prithivMLmods/DeepSeek-OCR-Latest-BF16.I64-v2.0" # - (https://huggingface.co/deepseek-ai/DeepSeek-OCR)
...
with gr.Blocks() as demo:
    gr.Markdown("# **DeepSeek OCR [exp] - [Model](https://huggingface.co/prithivMLmods/DeepSeek-OCR-Latest-BF16.I64)**", elem_id="main-title")
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            output_text = gr.Textbox(label="Output (OCR) [t >= 30 seconds]", lines=8, interactive=True)
            output_image = gr.Image(label="Layout Detection (If Any)", type="pil")
            
            with gr.Accordion("Note", open=False):
                gr.Markdown("Inference using Huggingface transformers on NVIDIA GPUs. This app is running with transformers version 4.57.6 and torch version 2.9.1.")
...
if __name__ == "__main__":
    demo.queue(max_size=20).launch(css=css, theme=steel_blue_theme, mcp_server=True, ssr_mode=False)
